...
def get_relevant_context(user_text: str, max_chars: int = 1500) -> str:
    """
    Find and format relevant past conversations for injection into system prompt.
    ...
    try:
        ...
        if not stats['ready'] or stats['indexed_conversations'] < 2:
            return ""
        ...
        for i, result in enumerate(results, 1):
            ...
            user = result.get('user_text', '')[:150]
            jarvis = result.get('jarvis_text', '')[:150]
        ...
        if "429" not in str(e):
            print(f"[Memory] ⚠️ Context retrieval failed: {e}")
        return ""